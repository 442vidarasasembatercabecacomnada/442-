...
Components.TitleBar = (function()
	...
	return function(Config)
		...
		local userPlan = "Freemium"
		local planTextColor = Color3.fromRGB(160, 160, 160)
		local isPremium = false

		if typeof(getgenv) == "function" and getgenv().__userplan == "Premium" then
			userPlan = "Premium"
		...
		local PlanLabel = New("TextLabel", {
			...
			TextSize = 13,
			TextColor3 = planTextColor,
			Size = UDim2.fromOffset(60, 20),
			Position = UDim2.new(0, 40, 0.48, 0), 
        ...
        if IconImageLabel then
            ...
            TweenService:Create(IconImageLabel, tweenInfo, {Rotation = 360}):Play()
        end
		
		if isPremium and gradientObj then
			local gradInfo = TweenInfo.new(2.5, Enum.EasingStyle.Circular, Enum.EasingDirection.InOut, -1)
			
			TweenService:Create(gradientObj, gradInfo, {Offset = Vector2.new(1, 0)}):Play()
		end
...
ElementsTable.Toggle = (function()
	...
	function Element:New(Idx, Config)
		...
		if not Mobile and Config.Keybind then
            ...
            -- !!! ESTA É A PARTE IMPORTANTE NOVA PARA O SAVEMANAGER FUNCIONAR !!! --
...
local Icons = {
	...
	["lucide-misc"] = "rbxassetid://117268085742685",
		...
		Toggle = {
			Save = function(idx, object) 
				return { 
					...
					idx = idx, 
					value = object.Value,
					key = object.Keybind and object.Keybind.Value or nil 
				} 
			end,
			Load = function(idx, data)
				if SaveManager.Options[idx] then 
					...
					if data.key and SaveManager.Options[idx].Keybind then
						if SaveManager.Options[idx].Keybind.SetKey then
							SaveManager.Options[idx].Keybind:SetKey(data.key)
						else
							SaveManager.Options[idx].Keybind.Value = data.key
						end
					end
				end
			end,
...
function Library:SetTheme(Value)
	...
	if Library.Window and Library.Window.Root:FindFirstChild("SnowContainer") then
		pcall(function() Library.Window.Root.SnowContainer:Destroy() end)
	end
	...
	if Library.Window and table.find(Library.Themes, Value) then
		...
		if Value == "supernova" and Library.Window then
			SnowActive = true
			...
			local SnowContainer = New("Frame", {
				...
				New("UICorner", { CornerRadius = UDim.new(0, 26) })
			})

			local flakes = {
				"rbxassetid://106223884088814", 
				"rbxassetid://106223884088814"
			}

			task.spawn(function()
				while SnowActive and Library.Window and SnowContainer.Parent do
					
					if Library.Window.Minimized or not getgenv().SnowEnabled then 
						task.wait(1)
						continue
					end

					local size = math.random(7, 14) 
					local startX = math.random() 
					
					local useImage = math.random() > 0.4
					local flake

					if useImage then
						flake = New("ImageLabel", {
							Parent = SnowContainer,
							BackgroundTransparency = 1,
							Image = flakes[math.random(1, #flakes)],
							ImageTransparency = math.random(40, 85)/100,
							ImageColor3 = Color3.fromRGB(255, 255, 255),
							Size = UDim2.fromOffset(size, size),
							AnchorPoint = Vector2.new(0.5, 0.5), 
							
							Position = UDim2.new(startX, 0, 0, -20), 
						...
						flake = New("Frame", {
							...
							Size = UDim2.fromOffset(math.floor(size/2.5), math.floor(size/2.5)),
							AnchorPoint = Vector2.new(0.5, 0.5),
					...
					local fallTime = math.random(50, 120) / 10
					local tweenInfo = TweenInfo.new(fallTime, Enum.EasingStyle.Linear)
					
					local goal = {
						Position = UDim2.fromScale(startX + (math.random(-5, 5)/100), 1.1),
						Rotation = math.random(-180, 180)
					}

					local tween = TweenService:Create(flake, tweenInfo, goal)
